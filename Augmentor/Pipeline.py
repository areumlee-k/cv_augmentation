                ...
                for i in range(len(images)):
                    if i == 0:
                        save_name = augmentor_image.class_label \
                                    + "_" \
                                    + os.path.basename(augmentor_image.image_path)[:-4] \
                                    + "_" \
                                    + "(original" \
                                    + ")" \
                                    + "." \
...
#     def scale_h(self, probability, scale_factor):
#         """
...
#         else:
#             self.add_operation(Scale_h(probability=probability, scale_factor=scale_factor))

    def scale_h(self, probability, min_scale_factor , max_scale_factor, num, binary_threshold):
        """
        Scale (enlarge) an image, while maintaining its aspect ratio. This
        returns an image with larger dimensions than the original image.
        ...
        if not 0 < probability <= 1:
            ...
            self.add_operation(Scale_h(probability, min_scale_factor , max_scale_factor, num,binary_threshold))
