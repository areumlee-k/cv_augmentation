...
class Scale_h(Operation):
    ...
    images.
    """
    def __init__(self, probability, min_scale_factor , max_scale_factor, num, binary_threshold):
        """
        ...
        if (num-1)!=0:
            addd = (max_scale_factor-min_scale_factor)/(num-1)
            
        arr = []
        for i in range(num):
            arr.append(float(min_scale_factor + (addd*i)))
        
        self.min_scale_factor = min_scale_factor
        self.max_scale_factor = max_scale_factor
        self.scale_factor = arr    
        self.num = num
...
#             return image.resize((new_w, new_h), resample=Image.BICUBIC)
        ...
        def do1(image, m,n):
            ...
            new_h = int(h * self.scale_factor[m])
            new_w = int(w * self.scale_factor[n])
        ...
        for image in images:
            for j in range(self.num):
                for u in range(self.num):
                    augmented_images.append(do3(image,j,u))
